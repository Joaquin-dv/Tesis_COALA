import { revisionSolicitada, solicitandoRevision, revisionYaSolicitada } from './modules/toastModule.js';
...
async function solicitarRevisionHumana(apunteId) {
	const response = await fetch(`api/index.php?model=Apuntes&method=updateSolicitoRevision&apunte_id=${apunteId}`);
	const data = await response.json();
	return data;
}
